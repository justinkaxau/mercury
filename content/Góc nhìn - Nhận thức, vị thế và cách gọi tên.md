...
```mermaid
flowchart TD
    A[Sự kiện / Hành vi khách quan] --> B[Khoảng cách quan sát]
    A --> C[Đóng khung ngôn ngữ]
    A --> D[Vị thế & Lợi ích]
    B --> E[Độ phóng đại cảm xúc]
    C --> F[Định hướng phán xét đạo đức]
    D --> G[Mức độ khắt khe của xã hội]
    E --> H[Kết luận chủ quan]
    F --> H
    G --> H
```
